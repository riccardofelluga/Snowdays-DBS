...
public class SponsorMenu extends Menu {
    ...
    private String firmsQuery = "SELECT s.company, s.cash, e.name, count(e.vat_no) AS quantity FROM (sponsor s LEFT JOIN edible e ON s.vat_no = e.vat_no) GROUP BY s.company, s.cash, e.name";
    private String sponsorshipQuery = "SELECT p.stud_id, p.name, p.surname, s.company FROM (participant p RIGHT JOIN finds f ON p.stud_id = f.stud_id) LEFT JOIN sponsor s ON f.vat_no = s.vat_no";
