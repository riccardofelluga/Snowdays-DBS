...
public class SportEventMenu extends Menu
{
    ...
    private String teamsQuery = "SELECT t.team_id, t.name, se.description FROM (team t LEFT JOIN clashes_in ci ON t.team_id = ci.team_id) LEFT JOIN sport_event se ON ci.sport_event_id = se.sport_event_id";
        ...
        MenuItem manageSportStaff = new MenuItem("See staff");
        manageSportStaff.setOnAction(e -> {
            DataHandler.getInstance().loadRemote(staffQuery, "manageStaff", true);
            ViewPane.getInstance().updateView();
        });

        MenuItem manageTeams = new MenuItem("See teams");
        manageTeams.setOnAction(e -> {
            DataHandler.getInstance().loadRemote(teamsQuery, "manageTeams", true);
            ViewPane.getInstance().updateView();
        });
        ...
        scoreboardTeam.setOnAction(e -> {
            ...
            DataHandler.getInstance().loadRemote("SELECT ci.placement, t.name FROM clashes_in ci LEFT JOIN team t ON ci.team_id = t.team_id WHERE sport_event_id = '" + ID + "' ORDER BY placement ASC", "teamSportsScoreboard", true);
            ViewPane.getInstance().updateView();
        });
